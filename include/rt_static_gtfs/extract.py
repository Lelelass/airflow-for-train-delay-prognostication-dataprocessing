...
def _transform_stockholm_tz(task_instance):
    data= task_instance.xcom_pull(
        task_ids = "extract_liseberg.extract_queue_time"
        )
    for ride in data:
        utc_time = datetime.datetime.strptime(ride['last_updated'], "%Y-%m-%dT%H:%M:%S.%fZ")
...
def extract_queue_time():
    extract_queue_time = PythonOperator(
        ...
        python_callable= _extract_queue_time, op_args =
        [theme_parks["Asterix"]], do_xcom_push =True
    )

    transform_timezone = PythonOperator(
        task_id="transform_stockholm_timezone",
        python_callable= _transform_stockholm_tz
        #smth missing ? args ?
    )
